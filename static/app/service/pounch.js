    ...
    .factory('pounch', ['$q','$filter','CONFIG', function ($q, $filter, CONFIG) {
        return {
            ...
            setTabs: (key, value, full_sync) => {
                ...
                if (full_sync) {
                    ...
                    db.destroy().then(_res => {
                        ...
                        db = new PouchDB('tabs', { revs_limit: 10, auto_compaction: true });
            ...
            alldocs: (name, include_docs) => {
                ...
                var deferred = $q.defer();
                new PouchDB(name, { revs_limit: 10, auto_compaction: true }).allDocs({
            ...
            getdocs: (keys) => {
                ...
                var deferred = $q.defer();
                new PouchDB('tabs', { revs_limit: 10, auto_compaction: true }).bulkGet({ docs: keys })
                    .then(deferred.resolve)
                    .catch(deferred.reject);
                return deferred.promise;
            },
            ...
            getbucket: (key,name) => {
                ...
                var deferred = $q.defer();
                new PouchDB('tabs', { revs_limit: 10, auto_compaction: true }).get(name)
                    .then(items => {
                        if (name == 'bucket_$$$'){
            ...
            setdoc: (name, key, value) => {
                var prepDoc = $filter('prepareDoc')(key, value);
                var deferred = $q.defer();
                var db = new PouchDB(name, { revs_limit: 10, auto_compaction: true });
                db.get(prepDoc._id).then(doc => {
                    db.put({
                        _id: doc._id,
                        _rev: doc._rev,
            ...
            setdocs: (docs) => {
                var deferred = $q.defer();
                new PouchDB('tabs', { revs_limit: 10, auto_compaction: true })
                    .bulkDocs(docs)
                    .then(deferred.resolve)
                    .catch(deferred.resolve);
                return deferred.promise;
            ...
            removeData: function (key) {
                console.log("Get");
                var deferred = $q.defer();
                var db = new PouchDB('test', { revs_limit: 10, auto_compaction: true });
                db.get(key)
